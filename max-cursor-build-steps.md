...
SocialProfile, and Note exactly as specified.
...
platform listed in each case's Social Profiles table. Add the five lightweight
queue-only entries from that doc as additional CaseSummary records (no full
scorecard needed for these — just enough to populate the queue view).
...
using the mock CaseSummary[] data. Columns: Influencer, Market, Status
(use StatusBadge), Deadline, Effort. Deadline should visually flag overdue
(coral) vs due-today (amber) vs on-track (neutral) per the PRD.
```
...
AI Analysis, Reports & Audit) per Dub's Sidebar Nav Item pattern.
```
...
(CitationBlock list), and the RiskLevelSelector. Sections should be visually
separated as standard cards per the Dub Dashboard Card pattern.
```

---

## Step 13 — AI Analysis page

```
Build the AI Analysis page per PRD section 4.3: mode toggle (Coverage Scan /
Scorecard Assembly), prompt chips including "Suggest risk levels," and static
mocked AIOutputCard results demonstrating both modes using the fictional case
data. No real API calls — hardcode representative output.
```

---

## Step 14 — Reports & Audit page
...
before/after) using mock activity data. Include filters for human actions vs AI runs.
```

---

## Step 15 — Wire routing and state
